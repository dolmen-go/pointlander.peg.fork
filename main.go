...
var (
	...
	modfile     = flag.String("modfile", "", "value for \"go tool -modfile=`MODFILE`\" in \"Code generated by\" line")
)
...
func main() {
	...
	err := parse(
		func(p *Peg[uint32], out io.Writer) error {
			...
			p.Strict = *strict
			if *modfile != "" {
				p.Tree.Generator = strings.Join(append([]string{"go", "tool", "-modfile", *modfile, "peg"}, os.Args[1:]...), " ")
			}
